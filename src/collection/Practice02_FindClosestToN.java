...
public class Practice02_FindClosestToN {
    public static void main(String[] args) {
        ...
        System.out.println(findClosest(new ArrayList<> (Arrays.asList(7, 7 ,7 ,7 )), 7));//RuntimeException
        System.out.println(findClosest(new ArrayList<> (Arrays.asList(3)), 3));//RuntimeException
        System.out.println(findClosest(new ArrayList<> (Arrays.asList()), 10));//RuntimeException
   ...
   [5, 4] 5 ->
    ...
    public static int findClosest(List<Integer> nums, int n){
        ...
        if (set.isEmpty() || (set.size() == 1 && set.first() == n))
            throw new RuntimeException("Ther is not number close to " + n + ".");
